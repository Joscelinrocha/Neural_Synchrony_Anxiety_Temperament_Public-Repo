...
# upload the datasets
#### Synchrony data
synch_data <- read_csv("data/neuro/wct_all-dyads_r56_byband.csv") %>%
  janitor::clean_names() %>%
  # select(-x1) %>%
  mutate(
...
# #### Synchrony data
# synch_data_real_dyads <- read_csv("data/neuro/wct_real-dyads_r56_byband.csv") %>%
#   janitor::clean_names() %>%
#   # select(-x1) %>%
#   mutate(
#     record_id = substr(as.character(parent), 1, 4),
...
synchrony_data_per_trial <- synch_data %>%
  mutate(
    ...
    category = factor(substr(parent, 1, 4) == substr(child, 1, 4),
                      levels = c(FALSE, TRUE), labels = c("0","1")),
    baseline = 0,

    avg_score = rowMeans(pick(s1_d1_hbo:s8_d4_hbo), na.rm = TRUE),

    DLPFC_avg_score = rowMeans(
      pick(s3_d2_hbo, s4_d2_hbo, s5_d3_hbo, s6_d3_hbo), na.rm = TRUE
    ),

    VLPFC_avg_score = rowMeans(
      pick(s1_d1_hbo, s2_d1_hbo, s2_d2_hbo, s7_d3_hbo, s7_d4_hbo, s8_d4_hbo),
      na.rm = TRUE
    )
  )
...
write_csv(synchrony_data_per_trial,"data/tidy/tidy_df_pseudo_and_real_per_trial.csv")
...
synchrony_data <- synch_data %>%
  # keep only what you need; be sure channel cols are numeric
  transmute(
    record_id, parent, child, block, band,
    across(s1_d1_hbo:s8_d4_hbo, as.numeric)
  ) %>%
  # per-(record_id, parent, child, block, band) means for each channel
  summarise(
    across(s1_d1_hbo:s8_d4_hbo, ~ mean(.x, na.rm = TRUE)),
    .by = c(record_id, parent, child, block, band)
  ) %>%
  mutate(
    category = factor(substr(parent, 1, 4) == substr(child, 1, 4),
                      levels = c(FALSE, TRUE), labels = c("0","1")),
    baseline = 0,
    avg_score = rowMeans(pick(s1_d1_hbo:s8_d4_hbo), na.rm = TRUE),
    DLPFC_avg_score = rowMeans(pick(s3_d2_hbo, s4_d2_hbo, s5_d3_hbo, s6_d3_hbo), na.rm = TRUE),
    VLPFC_avg_score = rowMeans(pick(s1_d1_hbo, s2_d1_hbo, s2_d2_hbo, s7_d3_hbo, s7_d4_hbo, s8_d4_hbo), na.rm = TRUE)
  )

synchrony_data$category <- recode_factor(synchrony_data$category, `1` = "Real Dyads", `0` = "Pseudo Dyads")

write_csv(synchrony_data,"data/tidy/tidy_df_pseudo_and_real.csv")
# write files in .Rdata format so the factors and other column types remain with the values I assigned
save(synchrony_data, file = "data/tidy/tidy_df_pseudo_and_real.RData")
```
## Temporary until I get pseudo particiants
## Finding the real dyads and Pseudo dyads

```{r}
# ### Creating variable to identify which ones are real dyads and which ones aren't
# synchrony_data_per_trial_real_dyads <- synch_data_real_dyads %>%
#   mutate(
#     # fully vectorized; no rowwise needed
#     category = factor(substr(parent, 1, 4) == substr(child, 1, 4),
#                       levels = c(FALSE, TRUE), labels = c("0","1")),
...
#   )
# 
# synchrony_data_per_trial_real_dyads$category <- recode_factor(synchrony_data_per_trial_real_dyads$category, `1` = "Real Dyads", `0` = "Pseudo Dyads")
# 
# write_csv(synchrony_data_per_trial_real_dyads,"data/tidy/tidy_df_real_per_trial.csv")
# # write files in .Rdata format so the factors and other column types remain with the values I assigned
# save(synchrony_data_per_trial_real_dyads, file = "data/tidy/tidy_df_real_per_trial.RData")
# 
# synchrony_data_real_dyads <- synch_data_real_dyads %>%
#   # keep only what you need; be sure channel cols are numeric
#   transmute(
#     record_id, parent, child, block, band,
#     across(s1_d1_hbo:s8_d4_hbo, as.numeric)
...
#   )
# 
# synchrony_data_real_dyads$category <- recode_factor(synchrony_data_real_dyads$category, `1` = "Real Dyads", `0` = "Pseudo Dyads")
# 
# write_csv(synchrony_data_real_dyads,"data/tidy/tidy_df_real.csv")
# # write files in .Rdata format so the factors and other column types remain with the values I assigned
# save(synchrony_data_real_dyads, file = "data/tidy/tidy_df_real.RData")
```
...
nirs_records_list <- nirs_records %>% pull()
...
## Adding Questionnaires (Temporary)
```{r}
# nirs_records_real_dyads <- synchrony_data_real_dyads %>%
#   dplyr::filter(band == "old") %>%
#   dplyr::select(record_id) %>%
#   unique()
# 
# nirs_records_list_real_dyads <- nirs_records_real_dyads %>% pull()
# 
# nirs_records_plus_demo <- nirs_records_real_dyads %>%
#   left_join(primary_parent_sample_long)
# 
# nirs_records_plus_qst_df <- nirs_records_real_dyads %>%
#   left_join(questionnaires_data)
# 
# write_csv(nirs_records_plus_qst_df, "data/tidy/pre_imputation_df.csv")
```
...
```{r}
dat_long_imp <- read_csv("data/tidy/post_imputation_df.csv")
```
...
### Child Anxiety
```{r}
hbq_items <- read_csv("questionnaires/input/hbq_items.csv") %>%
  janitor::clean_names()
