---
title: "Synchrony analysis DB-DOS"
author: "Joscelin Rocha-Hidalgo"
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
  pdf_document: default
editor_options: 
  markdown: 
    wrap: 72
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Libraries

```{r packages, include=FALSE}
# Load the libraries
library(tidyverse)
library(readxl)
library(eeptools)
library(lme4)
library(afex)
library(cowplot)
source("packages/R_rainclouds.R")
library(PerformanceAnalytics)
library(modelsummary)
library(ggrepel)
library(report)
library(car)
library(ggeffects)
library(NatParksPalettes)
# devtools::install_github("johannesbjork/LaCroixColoR")
# remotes::install_github("m-clark/NineteenEightyR")
# remotes::install_github("jorvlan/raincloudplots")
#library(NineteenEightyR)
library(PerformanceAnalytics)
library(gt)
library(sjPlot)
library(mice)
library(ggrain)
library(emmeans)
library(interactions)
library(Hmisc)

filter <- dplyr::filter
mutate <- dplyr::mutate
select <- dplyr::select
summarise <- dplyr::summarise
summarize <- dplyr::summarize

theme_apa <- papaja::theme_apa

# Define a function to check for specific NA patterns
check_sequence <- function(x) {
  na_pattern_1 <- is.na(x[1]) && !is.na(x[2]) && is.na(x[3]) && !is.na(x[4])
  na_pattern_2 <- !is.na(x[1]) && is.na(x[2]) && !is.na(x[3]) && is.na(x[4])
  na_pattern_3 <- !is.na(x[1]) && is.na(x[2]) && is.na(x[3]) && !is.na(x[4])
  na_pattern_4 <- is.na(x[1]) && !is.na(x[2]) && is.na(x[3]) && !is.na(x[4])
  return(na_pattern_1 || na_pattern_2 || na_pattern_3 || na_pattern_4)
}
```

## loading dfs

```{r}
load("data/tidy/tidy_df.RData")
#load("data/tidy/tidy_df_real.RData")
load("data/tidy/tidy_df_pseudo_and_real.RData")
#load("data/tidy/tidy_df_real_dyads.RData")
load("data/tidy/tidy_df_pseudo_and_real_per_trial.RData")
#load("data/tidy/tidy_df_real_per_trial.RData")
```

## Descriptives

### Num of dyads

```{r}
### Number of dyads per group (Pseudo = 0 vs Real = 1)
synchrony_data_per_trial|>
  mutate(unique_code = paste0(parent, child))|>
  distinct(unique_code, .keep_all = TRUE)|>
  group_by(category)|>
  count()|>
  ungroup()
```

### Missing Channels

```{r}
### Number of missing channels per group (Pseudo = 0 vs Real = 1 => Category) & Block (1,2,3)
## Values => if 1 = Missing, 0 = not missing
synchrony_data|>
  filter(band == "old") |> 
  mutate(unique_code = paste0(parent, child))|>
  select(parent:category)|>
  pivot_longer(cols = c(s1_d1_hbo:s8_d4_hbo), names_to = "channel", values_to = "values")|>
  mutate(
    values = case_when(
      is.na(values) ~ "1",
      TRUE ~ "0"
    ),
    values = as.character(values)
  )|>
  group_by(category, band,block, values)|>
  count()|>
  ungroup()

```

## Choose the band here

```{r}
synchrony_data <- synchrony_data |> 
  filter(band == "old")

# synchrony_data_real_dyads <- synchrony_data_real_dyads |>
#   filter(band == "old")
```

### Mean Synchrony per Block

#### Old Band

```{r}
### Mean synchrony per block for all Real Dyads
synchrony_data|>
  filter(category == "Real Dyads" & band == "old")|>
  group_by(block)|>
  summarise(mean = round(mean(avg_score, .na.rm = TRUE), 4))|>
  ungroup()
```

### Age

```{r}
### Age
nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE)|>
  summarise(
    child_age_mean = round(mean(c_age_y, na.rm = TRUE), 2),
    child_age_sd = round(sd(c_age_y, na.rm = TRUE), 2),
    child_age_min = round(min(c_age_y, na.rm = TRUE), 2),
    child_age_max = round(max(c_age_y, na.rm = TRUE), 2)
  )
```

### Location

```{r}
### Location
nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE)|>
  group_by(site)|>
  count()|>
  ungroup()
```

### Sex Parent

```{r}
### Sex
nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE)|>
  group_by(p_sex)|>
  count()|>
  ungroup()
```

### Sex Child

```{r}
### Sex Child
nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE)|>
  group_by(c_sex)|>
  count()|>
  ungroup()
```

### Race Child

```{r}
## Race
nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE)|>
  group_by(c_race)|>
  count()|>
  ungroup()|>
  filter(c_race != "None")|>
  mutate(percent = round(n / sum(n) * 100, 2))
```

### Race Parent

```{r}
## Race Parent

nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE)|>
  group_by(p_race)|>
  count()|>
  ungroup()|>
  filter(p_race != "None")|>
  mutate(percent = round(n / sum(n) * 100, 2))
```

### Latinx Child

```{r}
## Latinx
nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE)|>
  group_by(c_latinx)|>
  count()|>
  ungroup()|>
  filter(c_latinx != "None")|>
  mutate(percent = round(n / sum(n) * 100, 2))
```

### Latinx Parent

```{r}
## Latinx Parent
nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE)|>
  group_by(p_latinx)|>
  count()|>
  ungroup()|>
  filter(p_latinx != "None")|>
  mutate(percent = round(n / sum(n) * 100, 2))
```

### Family Income

```{r}
nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE)|>
  group_by(income_house)|>
  count()|>
  ungroup()|>
  filter(income_house != "None")|>
  mutate(percent = round(n / sum(n) * 100, 2))
```

### Parent Education

```{r}
nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE)|>
  group_by(p_edu)|>
  count()|>
  ungroup()|>
  filter(p_edu != "None")|>
  mutate(percent = round(n / sum(n) * 100, 2))
```

### Graph of real and Pseudo Dyads

#### PFC

```{r}
# synchrony_data$trial <- factor(synchrony_data$trial, levels = c("Trial 1", "Trial 2","Trial 3","Trial 4"))

synchrony_data <- synchrony_data|>
  mutate(new_block = case_when(
    block == "Block 1" ~ "Free Play Block",
    block == "Block 2" ~ "Stressful Block",
    block == "Block 3" ~ "Recovery Block",
    TRUE ~ "Error"
  ))

synchrony_data$new_block <- factor(synchrony_data$new_block, levels = c("Free Play Block", "Stressful Block", "Recovery Block"))

ggplot(synchrony_data, aes(x = category, y = avg_score, fill = category, col = category)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .2, adjust = 4) +
  geom_point(position = position_jitter(width = .10), size = 1, alpha = 0.4) +
  scale_fill_brewer(palette = "Dark2") +
  scale_colour_brewer(palette = "Dark2") +
  guides(fill = FALSE, col = FALSE) +
  geom_boxplot(aes(x = as.numeric(category) + 0.25, y = avg_score), outlier.shape = NA, alpha = 0.4, width = .1, colour = "BLACK") +
  ylab("Synchrony Values (PFC)") +
  xlab("Dyads Type") +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 1 / 4, color = "black") +
  stat_summary(fun = "mean", geom = "point", color = "black") +
  theme_apa() +
  facet_wrap(~new_block, nrow = 1) +
  theme(text = element_text(size = 30)) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 8))



ggsave("figures/aim_0_real_pseudo_dyads_avg-PFC.png", width = 14, height = 10)
```

#### DLPFC

```{r}

ggplot(synchrony_data, aes(x = category, y = DLPFC_avg_score, fill = category, col = category)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .2, adjust = 4) +
  geom_point(position = position_jitter(width = .10), size = 1, alpha = 0.4) +
  scale_fill_brewer(palette = "Dark2") +
  scale_colour_brewer(palette = "Dark2") +
  guides(fill = FALSE, col = FALSE) +
  geom_boxplot(aes(x = as.numeric(category) + 0.25, y = DLPFC_avg_score), outlier.shape = NA, alpha = 0.4, width = .1, colour = "BLACK") +
  ylab("Synchrony Values (DLPFC)") +
  xlab("Dyads Type") +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 1 / 4, color = "black") +
  stat_summary(fun = "mean", geom = "point", color = "black") +
  theme_apa() +
  facet_wrap(~new_block, nrow = 1) +
  theme(text = element_text(size = 20)) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 8))

ggsave("figures/aim_0_real_pseudo_dyads_avg-DLPFC.png", width = 14, height = 10)
```

#### VLPFC

```{r}
ggplot(synchrony_data, aes(x = category, y = VLPFC_avg_score, fill = category, col = category)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .2, adjust = 4) +
  geom_point(position = position_jitter(width = .10), size = 1, alpha = 0.4) +
  scale_fill_brewer(palette = "Dark2") +
  scale_colour_brewer(palette = "Dark2") +
  guides(fill = FALSE, col = FALSE) +
  geom_boxplot(aes(x = as.numeric(category) + 0.25, y = VLPFC_avg_score), outlier.shape = NA, alpha = 0.4, width = .1, colour = "BLACK") +
  ylab("Synchrony Values (VLPFC)") +
  xlab("Dyads Type") +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 1 / 4, color = "black") +
  stat_summary(fun = "mean", geom = "point", color = "black") +
  theme_apa() +
  facet_wrap(~new_block, nrow = 1) +
  theme(text = element_text(size = 20)) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 8))

ggsave("figures/aim_0_real_pseudo_dyads_avg-VLPFC.png", width = 14, height = 10)
```

### Model Testing Pseudo and real dyads

```{r}
synchrony_data_per_trial$block <- factor(synchrony_data_per_trial$block, levels = c("Block 1", "Block 2", "Block 3"))

synchrony_data_per_trial <- synchrony_data_per_trial|>
  mutate(new_id = paste0(parent, child),
         trial_num = parse_number(trial)) |> 
  filter(band == "old")


model1 <- lmer(
  avg_score ~ category + (1 + block + trial_num | new_id),
  data = synchrony_data_per_trial,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model1)
report(model1)


model1.2 <- lmer(
  avg_score ~ category + (1 + block | new_id),
  data = synchrony_data_per_trial,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model1.2)
report(model1.2)

anova(model1.2, model1)

model1.3 <- lmer(
  avg_score ~ category*block + (1 + block + trial_num | new_id),
  data = synchrony_data_per_trial,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model1.3)

```

#### Table Summary

```{r}
modelsummary_1 <- list(
  "Full Model" = model1
)

msummary(modelsummary_1,
  output = "tables/aim_0.docx",
  stars = TRUE, align = "lc", metrics = c("RMSE", "R2", "AIC", "BIC", "Log.Lik.", "F")
)
```

### Sum of blank channels

```{r}
synchrony_data|>
  filter(category == "Real Dyads")|>
  dplyr::mutate(across(c("s1_d1_hbo":"s8_d4_hbo"), as.numeric))|>
  rowwise()|>
  mutate(num_na = sum(is.na(c_across(s1_d1_hbo:s8_d4_hbo))))|>
  group_by(block)|>
  summarise(num_na = sum(num_na, na.rm = TRUE))|>
  ungroup()
```

### Adding Lab ID

1 = WUSTL 0 = PSU

```{r}
# nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = #c("Block 2", "Block 1", "Block 3"))
nirs_per_trial_plus_qst <- nirs_per_trial_plus_qst|>
  mutate(
    block_2 = case_when(
      block == "Block 1" ~ 1,
      block == "Block 2" ~ 2,
      block == "Block 3" ~ 3
    ),
    block_sq = block_2^2
  ) %>%
  dplyr::filter(band == "old")


nirs_per_trial_plus_qst <- nirs_per_trial_plus_qst|>
  mutate(lab = case_when(
    record_id >= 1200 ~ 0,
    TRUE ~ 1
  ),
  trial_num = parse_number(trial))
```

### Breaking apart datasets

```{r}
block1 <- nirs_per_trial_plus_qst|>
  filter(block == "Block 1")

block2 <- nirs_per_trial_plus_qst|>
  filter(block == "Block 2")

block3 <- nirs_per_trial_plus_qst|>
  filter(block == "Block 3")
```

## Correlations

p-values(0, 0.001, 0.01, 0.05, 0.1, 1) \<=\> symbols(“***”, “**”, “*”,
“.”, " “)

```{r}
correlation_df <- block1|>
  select(stai_trait, stai_state, cbq_effortful_control, cbq_fear, anxiety_c)

chart.Correlation(correlation_df, histogram = TRUE, method = "pearson")
```

### Correlations-block 1 neuro

```{r}
correlation_df <- block1|>
  select(stai_trait, stai_state, cbq_effortful_control, cbq_fear, anxiety_c, avg_score, DLPFC_avg_score, VLPFC_avg_score)

chart.Correlation(correlation_df, histogram = TRUE, method = "pearson")
```

### Correlations-block 2 neuro

```{r}
correlation_df <- block2|>
  select(stai_trait, stai_state, cbq_effortful_control, cbq_fear, anxiety_c, avg_score, DLPFC_avg_score, VLPFC_avg_score)

chart.Correlation(correlation_df, histogram = TRUE, method = "pearson")
```

### Correlations-block 3 neuro

```{r}
correlation_df <- block3|>
  select(stai_trait, stai_state, cbq_effortful_control, cbq_fear, anxiety_c, avg_score, DLPFC_avg_score, VLPFC_avg_score)

chart.Correlation(correlation_df, histogram = TRUE, method = "pearson")
```

### Multilevel Correlations

```{r message=FALSE, warning=FALSE}
mutilevel_cor_data <- nirs_per_trial_plus_qst|>
  select(block, trial, cbq_fear, cbq_effortful_control, anxiety_c, stai_state, stai_trait, avg_score, DLPFC_avg_score, VLPFC_avg_score)|>
  mutate(
    block = as_factor(block),
    trial = as_factor(trial)
  )

correlation::correlation(mutilevel_cor_data, multilevel = TRUE)
```

## Lab Comparisons

there is no significant interaction between lab and blocks.

```{r}
model0 <- lmer(avg_score ~ site + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)
summary(model0)
report(model0)

model00 <- lmer(DLPFC_avg_score ~ site + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)
summary(model00)
report(model00)


model000 <- lmer(VLPFC_avg_score ~ site + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)
summary(model000)
report(model000)
```

## 1. Aim 1: To examine parent-child neural synchrony using the DBDOS BioSync version

### Main Models

#### PFC

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 3", "Block 1", "Block 2"))

model1a <-lmer(
  avg_score ~
    c_age_y_centered + block + c_sex + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)
summary(model1a)
report(model1a)




nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 2", "Block 1", "Block 3"))

model1b <- lmer(
  avg_score ~
    c_age_y_centered + block + c_sex + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)
summary(model1b)
report(model1b)

performance::check_outliers(model1b)

##### Slope of EC for each block

# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")

#contrast(slopes, "poly") 
## marginal means by Block and FDR-adjusted pairwise tests
emm  <- emmeans(model1a, ~ block)
pairs_block <- pairs(emm, adjust = "none")
summary(emm)
summary(pairs_block)
confint(pairs_block) 
```

#### DLPFC

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 1", "Block 2", "Block 3"))

model2 <- lmer(
  DLPFC_avg_score ~
    c_age_y_centered + block + c_sex +  (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model2)
report(model2)

performance::check_outliers(model2)

nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 2", "Block 1", "Block 3"))

model2b <- lmer(
  DLPFC_avg_score ~
    c_age_y_centered + block + c_sex + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model2b)
report(model2b)

performance::check_outliers(model2b)


## marginal means by Block and FDR-adjusted pairwise tests
emm  <- emmeans(model2, ~ block)
pairs_block <- pairs(emm, adjust = "none")
summary(emm)
summary(pairs_block)
confint(pairs_block) 
```

#### VLPFC

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 1", "Block 2", "Block 3"))

model3 <- lmer(
  VLPFC_avg_score ~
    c_age_y_centered + block + c_sex  + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model3)
report(model3)

performance::check_outliers(model3)

## marginal means by Block and FDR-adjusted pairwise tests
emm  <- emmeans(model3, ~ block)
pairs_block <- pairs(emm, adjust = "none")
summary(emm)
summary(pairs_block)
confint(pairs_block)

nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 2", "Block 1", "Block 3"))

model3b <- lmer(
  VLPFC_avg_score ~
    c_age_y_centered + block + c_sex  + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model3b)
report(model3b)
```

### PFC Graph

```{r}
nirs_per_trial_plus_qst <- nirs_per_trial_plus_qst|>
  mutate(new_block = case_when(
    block == "Block 1" ~ "Free Play Block",
    block == "Block 2" ~ "Stressful Block",
    block == "Block 3" ~ "Recovery Block",
    TRUE ~ "Error"
  ),
  new_block = as.factor(new_block)
  )

nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 1", "Block 2", "Block 3"))
nirs_per_trial_plus_qst$new_block <- factor(nirs_per_trial_plus_qst$new_block, levels = c("Free Play Block", "Stressful Block", "Recovery Block"))


nirs_per_trial_plus_qst %>%
  group_by(record_id,block,new_block) %>%
  dplyr::summarise(avg_score = mean(avg_score, na.rm = TRUE)) %>%
  ungroup() %>%
  ggplot(., aes(x = new_block, y = avg_score, fill = new_block, col = new_block)) +
  scale_color_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B")
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B")
  ) +
  guides(fill = FALSE, col = FALSE) +
  geom_rain(alpha = .5, 
            rain.side = "f2x2", 
            id.long.var = "record_id",
            quantiles = NULL) +
  ylab("Neural Synchrony Values (PFC)") +
  xlab("Block Type") +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 1 / 4, color = "black") +
  stat_summary(fun = "mean", geom = "point", color = "black") +
  theme_apa() +
  theme(text = element_text(size = 15, family="Arial"))


ggsave("figures/aim_1_block_synchrony_PFC.png", width = 7, height = 5)
```

### DLPFC Graph

```{r}
nirs_per_trial_plus_qst%>%
  group_by(record_id,block,new_block) %>% 
  dplyr::summarise(DLPFC_avg_score = mean(DLPFC_avg_score, na.rm = TRUE)) %>% 
  ungroup()%>%
  ggplot(., aes(x = new_block, y = DLPFC_avg_score, fill = new_block, col = new_block)) +
  scale_color_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B")
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B")
  ) +
  guides(fill = FALSE, col = FALSE) +
  geom_rain(alpha = .5, 
            rain.side = "f2x2", 
            id.long.var = "record_id",
            quantiles = NULL) +
  ylab("Neural Synchrony Values (DLPFC)") +
  xlab("Play Block") +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 1 / 4, color = "black") +
  stat_summary(fun = "mean", geom = "point", color = "black") +
  theme_apa() +
  theme(text = element_text(size = 15, family="Arial"))


ggsave("figures/aim_1_block_synchrony_DLPFC.png", width = 7, height = 5)
```

### VLPFC Graph

```{r}
nirs_per_trial_plus_qst%>%
  group_by(record_id,block,new_block) %>%
  dplyr::summarise(VLPFC_avg_score = mean(VLPFC_avg_score, na.rm = TRUE)) %>%
  ungroup()%>%
  ggplot(., aes(x = new_block, y = VLPFC_avg_score, fill = new_block, col = new_block)) +
  scale_color_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B")
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B")
  ) +
  guides(fill = FALSE, col = FALSE) +
  geom_rain(alpha = .5, 
            rain.side = "f2x2", 
            id.long.var = "record_id",
            quantiles = NULL) +
  ylab("Neural Synchrony Values (VLPFC)") +
  xlab("Play Block") +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 1 / 4, color = "black") +
  stat_summary(fun = "mean", geom = "point", color = "black") +
  theme_apa() +
  theme(text = element_text(size = 15, family="Arial"))


ggsave("figures/aim_1_block_synchrony_VLPFC.png", width = 7, height = 5)
```

### Graph per Trial

Block 1: - Trial 1: Markers (fine) - Trial 2: Markers (dots) - Trial 3:
Stickers (shapes) - Trial 4: Stickers (strips)

Block 2: - Trial 1: Puzzle set 1 - Trial 2: Puzzle set 2 - Trial 3:
Puzzle set 3 - Trial 4: Puzzle set 4

Block 3: - Trial 1: Magnatiles - Trial 2: Human and Animal connectors -
Trial 3: Animals, trees, and fences - Trial 4: Cars, trucks

```{r}
synchrony_data_per_trial$trial <- as.factor(synchrony_data_per_trial$trial)
synchrony_data_per_trial$block <- as.factor(synchrony_data_per_trial$block)

synchrony_data_per_trial$block <- factor(synchrony_data_per_trial$block, levels = c("Block 1", "Block 2", "Block 3"))

nirs_per_trial_plus_qst %>%
  filter(category == "Real Dyads")%>%
  ggplot(., aes(x = trial, y = avg_score, fill = trial, col = trial)) +
  scale_fill_brewer(palette = "Dark2") +
  scale_colour_brewer(palette = "Dark2") +
  guides(fill = FALSE, col = FALSE) +
  geom_rain(alpha = .5, rain.side = "l", id.long.var = "record_id") +
  ylab("Neural Synchrony Values") +
  xlab("Play Block") +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 1 / 4, color = "black", na.rm = TRUE) +
  stat_summary(fun = "mean", geom = "point", color = "black", na.rm = TRUE) +
  theme_apa() +
  theme(text = element_text(size = 20)) +
  facet_wrap(~new_block) +
  scale_y_continuous(limits = c(0.2, .42), breaks = seq(0.2, .4, by = 0.10)) # Setting Y-axis limits and ticks

ggsave("figures/aim_1_block_synchrony_per-trial_PFC.png", width = 14, height = 10)


not_in_stream_df <- nirs_per_trial_plus_qst|>
  filter(category == "Real Dyads")|>
  select(record_id, block, trial, DLPFC_avg_score)|>
  pivot_wider(
    names_from = trial,
    values_from = DLPFC_avg_score
  )|>
  rowwise()|>
  mutate(
    pattern_found = check_sequence(c(`Trial 1`, `Trial 2`, `Trial 3`, `Trial 4`)),
    pattern_found = case_when(
      sum(is.na(c(`Trial 1`, `Trial 2`, `Trial 3`, `Trial 4`))) == 3 ~ TRUE,
      TRUE ~ pattern_found
    )
  )|>
  ungroup()|>
  filter(pattern_found)|>
  pivot_longer(cols = c("Trial 1":"Trial 4"), values_to = "DLPFC_avg_score", names_to = "trial")|>
  filter(!is.na(DLPFC_avg_score))


nirs_per_trial_plus_qst%>%
  filter(category == "Real Dyads")%>%
  ggplot(., aes(x = trial, y = DLPFC_avg_score, fill = trial, col = trial)) +
  scale_fill_brewer(palette = "Dark2") +
  scale_colour_brewer(palette = "Dark2") +
  guides(fill = FALSE, col = FALSE) +
  geom_rain(alpha = .5, rain.side = "l", id.long.var = "record_id") +
  geom_point(data = not_in_stream_df, alpha = .5) +
  ylab("Neural Synchrony Values (DLPFC)") +
  xlab("Play Block") +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 1 / 4, color = "black", na.rm = TRUE) +
  stat_summary(fun = "mean", geom = "point", color = "black", na.rm = TRUE) +
  theme_apa() +
  theme(text = element_text(size = 20)) +
  facet_wrap(~new_block) +
  scale_y_continuous(limits = c(0.2, .4), breaks = seq(0.2, .4, by = 0.10)) # Setting Y-axis limits and ticks


ggsave("figures/aim_1_block_synchrony_per-trial_DLPFC.png", width = 14, height = 10)

not_in_stream_df <- nirs_per_trial_plus_qst|>
  filter(category == "Real Dyads")|>
  select(record_id, block, trial, VLPFC_avg_score)|>
  pivot_wider(
    names_from = trial,
    values_from = VLPFC_avg_score
  )|>
  rowwise()|>
  mutate(
    pattern_found = check_sequence(c(`Trial 1`, `Trial 2`, `Trial 3`, `Trial 4`)),
    pattern_found = case_when(
      sum(is.na(c(`Trial 1`, `Trial 2`, `Trial 3`, `Trial 4`))) == 3 ~ TRUE,
      TRUE ~ pattern_found
    )
  )|>
  ungroup()|>
  filter(pattern_found)|>
  pivot_longer(cols = c("Trial 1":"Trial 4"), values_to = "VLPFC_avg_score", names_to = "trial")|>
  filter(!is.na(VLPFC_avg_score))

nirs_per_trial_plus_qst%>%
  filter(category == "Real Dyads")%>%
  ggplot(., aes(x = trial, y = VLPFC_avg_score, fill = trial, col = trial)) +
  scale_fill_brewer(palette = "Dark2") +
  scale_colour_brewer(palette = "Dark2") +
  guides(fill = FALSE, col = FALSE) +
  geom_rain(alpha = .5, rain.side = "l", id.long.var = "record_id") +
  geom_point(data = not_in_stream_df, alpha = .5) +
  ylab("Neural Synchrony Values (VLPFC)") +
  xlab("Play Block") +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 1 / 4, color = "black") +
  stat_summary(fun = "mean", geom = "point", color = "black") +
  theme_apa() +
  theme(text = element_text(size = 20)) +
  facet_wrap(~new_block) +
  scale_y_continuous(limits = c(0.2, .4), breaks = seq(0.20, .4, by = 0.10)) # Setting Y-axis limits and ticks
# 
# 
ggsave("figures/aim_1_block_synchrony_per-trial_VLPFC.png", width = 14, height = 10)
```

## Aim 2: To examine the relationship between child temperament and dyadic neural synchrony

### PFC

#### Model -block 2 reference

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 3", "Block 2", "Block 1"))

model1 <- lmer(
  avg_score ~ c_age_y_centered + c_sex + cbq_fear * block + cbq_effortful_control * block  + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model1)



# emm = emmeans(model1, ~ cbq_effortful_control*block )
# pairs(emm)
# # or for simple comparisons
# #pairs(emm, simple = "each")

#anova(model1, type = "III")
#report(model1)

performance::check_outliers(model1)

nirs_per_trial_plus_qst$block_numeric <- as.numeric(nirs_per_trial_plus_qst$block)

nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 1", "Block 2", "Block 3"))


model1b <- lmer(
  avg_score ~ c_age_y_centered + c_sex + cbq_fear * block + cbq_effortful_control * block  + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model1b)
report(model1)
```

##### Report

```{r}
tab_model(model1, p.val = "kr", show.df = TRUE)
```

#### Simple intercepts and slopes

```{r}
sim_slopes(model1,
  pred = cbq_effortful_control, modx = block, cond.int = TRUE,
  centered = "none", johnson_neyman = FALSE
)
```

##### Slope of EC for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model1, ~ block, var = "cbq_effortful_control")
summary(slopes,, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  
contrast(slopes, "poly") 
```

##### Slope of Fear for each block

```{r}
# Slope of EC for each block
slopes <- emtrends(model1, specs = ~ block, var = "cbq_fear")
summary(slopes)

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
```

#### Interaction Plot

```{r}
interact_plot(model1,
  pred = cbq_effortful_control, modx = block,
  x.label = "Effortful Control", y.label = "PFC Synchrony Scores",
  legend.main = "Block"
)

interact_plot(model1,
  pred = cbq_fear, modx = block,
  x.label = "Fear", y.label = "PFC Synchrony Scores",
  legend.main = "Block"
)
```

#### Plot for interaction: EC*Block

```{r}
interaction_eff1 <- ggpredict(model1, c("cbq_effortful_control", "block"))
interaction_eff1

interaction_eff1$group <- factor(interaction_eff1$group, levels = c("Block 1", "Block 2", "Block 3"))

ggplot(interaction_eff1, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.1) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  theme(text = element_text( family="Arial")) +
  scale_y_continuous(name = "PFC Neural Synchrony (predicted)", limits = c(0.22, .3), breaks = seq(0.22, .3, by = 0.02)) + # Setting Y-axis limits and ticks
  scale_x_continuous(name = "Effortful Control (CBQ scores)", limits = c(3, 6), breaks = seq(3, 6, by = 1)) # Setting Y-axis limits and ticks


ggsave("figures/aim_2_EC-Block_PFC.png", width = 10, height = 8)
```

#### Plot for interaction: F\*Block

```{r}
interaction_eff1b <- ggpredict(model1, c("cbq_fear", "block"))
interaction_eff1b

interaction_eff1b$group <- factor(interaction_eff1b$group, levels = c("Block 1", "Block 2", "Block 3"))

ggplot(interaction_eff1b, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.1) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Fear (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
    theme(text = element_text(family="Arial")) +
  scale_y_continuous(name = "PFC Neural Synchrony (predicted)", limits = c(0.22, .3), breaks = seq(0.22, .3, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_2_F-Block_PFC.png", width = 10, height = 8)
```

### DLPFC

#### Model-block 2 reference

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 3", "Block 1", "Block 2"))

model2 <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block  + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model2)
report(model2)

emm <- emmeans(model2, ~ cbq_effortful_control * block)
pairs(emm)
# or for simple comparisons
pairs(emm, simple = "each")
```

#### Model-block 1 reference

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 1", "Block 2", "Block 3"))

model2b <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model2b)
report(model2b)



emm <- emmeans(model2b, ~ cbq_effortful_control * block)
pairs(emm)
# or for simple comparisons
# pairs(emm, simple = "each")

report(anova(model2b, type = "III"))
```

##### Slope of EC for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model2b, ~ block, var = "cbq_effortful_control")
summary(slopes,, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
contrast(slopes, "poly") 
```

##### Slope of Fear for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model2b, ~ block, var = "cbq_fear")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
contrast(slopes, "poly") 
```

#### Simple intercepts & slopes: EC*Block

```{r}
interaction_eff2 <- ggpredict(model2b, c("cbq_effortful_control", "block"))
interaction_eff2

interaction_eff2$group <- factor(interaction_eff2$group, levels = c("Block 1", "Block 2", "Block 3"))

ggplot(interaction_eff2, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Effortful Control (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  )  +
    theme(text = element_text(family="Arial")) +
  scale_y_continuous(name = "DLPFC Neural Synchrony (predicted)", limits = c(0.22, .3), breaks = seq(0.22, .3, by = 0.02)) # Setting Y-axis limits and ticks

ggsave("figures/aim_2_EC-Block_DLPFC.png", width = 10, height = 8)
```

#### Simple intercepts & slopes: F\*Block

```{r}
interaction_eff3 <- ggpredict(model2b, c("cbq_fear", "block"))
interaction_eff3

interaction_eff3$group <- factor(interaction_eff3$group, levels = c("Block 1", "Block 2", "Block 3"))

ggplot(interaction_eff3, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Fear (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
    theme(text = element_text(family="Arial")) +
  scale_y_continuous(name = "DLPFC Neural Synchrony (predicted)", limits = c(0.22, .3), breaks = seq(0.22, .3, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_2_F-Block_DLPFC.png", width = 10, height = 8)
```

### VLPFC

#### Model- block 2 reference

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 3", "Block 1", "Block 2"))

model3 <- lmer(
  VLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block  + (1 + block +trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model3)
performance::check_outliers(model3)

emm <- emmeans(model3, ~ cbq_effortful_control * block)
pairs(emm)
# or for simple comparisons
# pairs(emm, simple = "each")

emm <- emmeans(model3, ~ cbq_fear * block)
pairs(emm)

report(model3)

nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 1", "Block 2", "Block 3"))

model3b <- lmer(
  VLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block  + (1 + block +trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model3b)
report(model3b)
```

#### Simple intercepts & slops: EC\*Block

```{r}
interaction_eff4 <- ggpredict(model3, c("cbq_effortful_control", "block"))
interaction_eff4

interaction_eff4$group <- factor(interaction_eff4$group, levels = c("Block 1", "Block 2", "Block 3"))


ggplot(interaction_eff4, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Effortful Control (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
    theme(text = element_text(family="Arial")) +
  scale_y_continuous(name = "VLPFC Neural Synchrony (predicted)", limits = c(0.22, .3), breaks = seq(0.22, .3, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_2_EC-Block_VLPFC.png.png", width = 10, height = 8)
```

#### Simple intercepts & slopes: F\*Block

```{r}
interaction_eff5 <- ggpredict(model3, c("cbq_fear", "block"))
interaction_eff5

interaction_eff5$group <- factor(interaction_eff5$group, levels = c("Block 1", "Block 2", "Block 3"))

ggplot(interaction_eff5, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = c("#C85729", "#127088", "#92874B"),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Fear (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
    theme(text = element_text(family="Arial")) +
  scale_y_continuous(name = "VLPFC Neural Synchrony (predicted)", limits = c(0.22, .3), breaks = seq(0.22, .3, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_2_F-Block_VLPFC.png", width = 10, height = 8)
```

##### Slope of EC for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3, ~ block, var = "cbq_effortful_control")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
contrast(slopes, "poly") 
```

##### Slope of Fear for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3, ~ block, var = "cbq_fear")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
contrast(slopes, "poly") 
```

### Report

```{r}
tab_model(model1, model2, model3, p.val = "kr", show.df = TRUE, collapse.ci = TRUE, p.style = "stars", digits = 4)
```

## 3. To examine the relationship between child temperament, anxiety, and dyadic neural synchrony

### Descriptives

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 1", "Block 2", "Block 3"))

descriptives_a <- nirs_per_trial_plus_qst %>%
  select(record_id, block, trial, avg_score, DLPFC_avg_score, VLPFC_avg_score) %>%
  group_by(block) %>%
  summarise(
    PFC_mean = round(mean(avg_score, na.rm = TRUE), 3),
    DLPFC_mean = round(mean(DLPFC_avg_score, na.rm = TRUE), 3),
    VLPFC_mean = round(mean(VLPFC_avg_score, na.rm = TRUE), 3),
    PFC_sd = round(sd(avg_score, na.rm = TRUE), 3),
    DLPFC_sd = round(sd(DLPFC_avg_score, na.rm = TRUE), 3),
    VLPFC_sd = round(sd(VLPFC_avg_score, na.rm = TRUE), 3),
    PFC_min = round(min(avg_score, na.rm = TRUE), 3),
    DLPFC_min = round(min(DLPFC_avg_score, na.rm = TRUE), 3),
    VLPFC_min = round(min(VLPFC_avg_score, na.rm = TRUE), 3),
    PFC_max = round(max(avg_score, na.rm = TRUE), 3),
    DLPFC_max = round(max(DLPFC_avg_score, na.rm = TRUE), 3),
    VLPFC_max = round(max(VLPFC_avg_score, na.rm = TRUE), 3)
  ) %>%
  ungroup() %>%
  pivot_longer(PFC_mean:VLPFC_max, names_to = c("ROI", "stat"), names_sep = "_", values_to = "value") %>%
  arrange(ROI) %>%
  unite("name", c("ROI", "block"))


nirs_per_trial_plus_qst %>%
  distinct(record_id,.keep_all = TRUE) %>%
  select(stai_trait, stai_state, cbq_effortful_control, cbq_fear, anxiety_c) %>%
  pivot_longer(stai_trait:anxiety_c, names_to = "name", values_to = "value") %>%
  group_by(name) %>%
  summarize(
    mean = round(mean(value, na.rm = TRUE), 3),
    sd = round(sd(value, na.rm = TRUE), 3),
    min = round(min(value, na.rm = TRUE), 3),
    max = round(max(value, na.rm = TRUE), 3)
  ) %>%
  pivot_longer(mean:max, names_to = "stat", values_to = "value") %>%
  rbind(descriptives_a) %>%
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(
    "Mean (SD)" = paste0(mean, " (", sd, ")"),
    Range = paste0(min, "-", max),
    name = as.factor(name)
  ) %>%
  select(name, "Mean (SD)", "Range") %>%
  gt()
```

### Correlations
#### Overall
```{r}
avg <- nirs_per_trial_plus_qst|>
  select(record_id, block,avg_score, DLPFC_avg_score,VLPFC_avg_score) |> 
  group_by(record_id,block) |> 
  summarise(average = mean(avg_score, na.rm = TRUE),
            dlpfc_average = mean(DLPFC_avg_score, na.rm = TRUE),
            vlpfc_average = mean(VLPFC_avg_score, na.rm = TRUE)) |> 
  ungroup() |> 
  pivot_wider(names_from = "block", values_from = c("average","dlpfc_average","vlpfc_average"), names_prefix = "synchrony_") |> 
  janitor::clean_names()
  

corr_df <- nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = FALSE) |> 
  left_join(avg)|> 
  select(-c(record_id)) 

# ++++++++++++++++++++++++++++
# flattenCorrMatrix
# ++++++++++++++++++++++++++++
# cormat : matrix of the correlation coefficients
# pmat : matrix of the correlation p-values
flattenCorrMatrix <- function(cormat, pmat) {
  ut <- upper.tri(cormat)
  data.frame(
    row = rownames(cormat)[row(cormat)[ut]],
    column = rownames(cormat)[col(cormat)[ut]],
    cor = round((cormat)[ut], 2),
    p = round(pmat[ut],9)
  )
}
res2 <- rcorr(as.matrix(corr_df))
res3 <- flattenCorrMatrix(res2$r, res2$P)

res3|>
  mutate(test = paste0("r = ", cor, ", p = ", p))|>
  select(row, column, test)|>
  pivot_wider(names_from = column, values_from = test)
```

#### Table with PFC

```{r}
avg <- nirs_per_trial_plus_qst|>
  group_by(record_id,block) |> 
  summarise(average = mean(avg_score, na.rm = TRUE)) |> 
  ungroup() |> 
  pivot_wider(names_from = "block", values_from = "average", names_prefix = "synchrony_") |> 
  janitor::clean_names()
  

corr_df <- nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE) |> 
  left_join(avg)|> 
  select(stai_trait, stai_state, cbq_effortful_control, cbq_fear, anxiety_c, c_age_m,synchrony_block_1,synchrony_block_2,synchrony_block_3) 

# ++++++++++++++++++++++++++++
# flattenCorrMatrix
# ++++++++++++++++++++++++++++
# cormat : matrix of the correlation coefficients
# pmat : matrix of the correlation p-values
flattenCorrMatrix <- function(cormat, pmat) {
  ut <- upper.tri(cormat)
  data.frame(
    row = rownames(cormat)[row(cormat)[ut]],
    column = rownames(cormat)[col(cormat)[ut]],
    cor = round((cormat)[ut], 2),
    p = round(pmat[ut],9)
  )
}
res2 <- rcorr(as.matrix(corr_df))
res3 <- flattenCorrMatrix(res2$r, res2$P)

res3|>
  mutate(test = paste0("r = ", cor, ", p = ", p))|>
  select(row, column, test)|>
  pivot_wider(names_from = column, values_from = test)
```
#### Table with DLPFC

```{r}
dlpfc_avg <- nirs_per_trial_plus_qst|>
  group_by(record_id,block) |> 
  summarise(average = mean(DLPFC_avg_score, na.rm = TRUE)) |> 
  ungroup() |> 
  pivot_wider(names_from = "block", values_from = "average", names_prefix = "synchrony_") |> 
  janitor::clean_names()
  

corr_df <- nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE) |> 
  left_join(dlpfc_avg)|> 
  select(stai_trait, stai_state, cbq_effortful_control, cbq_fear, anxiety_c, c_age_m,synchrony_block_1,synchrony_block_2,synchrony_block_3) 

# ++++++++++++++++++++++++++++
# flattenCorrMatrix
# ++++++++++++++++++++++++++++
# cormat : matrix of the correlation coefficients
# pmat : matrix of the correlation p-values
flattenCorrMatrix <- function(cormat, pmat) {
  ut <- upper.tri(cormat)
  data.frame(
    row = rownames(cormat)[row(cormat)[ut]],
    column = rownames(cormat)[col(cormat)[ut]],
    cor = round((cormat)[ut], 2),
    p = round(pmat[ut],9)
  )
}
res2 <- rcorr(as.matrix(corr_df))
res3 <- flattenCorrMatrix(res2$r, res2$P)

res3|>
  mutate(test = paste0("r = ", cor, ", p = ", p))|>
  select(row, column, test)|>
  pivot_wider(names_from = column, values_from = test)
```
#### Table with VLPFC

```{r}
vlpfc_avg <- nirs_per_trial_plus_qst|>
  group_by(record_id,block) |> 
  summarise(average = mean(VLPFC_avg_score, na.rm = TRUE)) |> 
  ungroup() |> 
  pivot_wider(names_from = "block", values_from = "average", names_prefix = "synchrony_") |> 
  janitor::clean_names()
  

corr_df <- nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE) |> 
  left_join(vlpfc_avg)|> 
  select(stai_trait, stai_state, cbq_effortful_control, cbq_fear, anxiety_c, c_age_m,synchrony_block_1,synchrony_block_2,synchrony_block_3) 

# ++++++++++++++++++++++++++++
# flattenCorrMatrix
# ++++++++++++++++++++++++++++
# cormat : matrix of the correlation coefficients
# pmat : matrix of the correlation p-values
flattenCorrMatrix <- function(cormat, pmat) {
  ut <- upper.tri(cormat)
  data.frame(
    row = rownames(cormat)[row(cormat)[ut]],
    column = rownames(cormat)[col(cormat)[ut]],
    cor = round((cormat)[ut], 2),
    p = round(pmat[ut],9)
  )
}
res2 <- rcorr(as.matrix(corr_df))
res3 <- flattenCorrMatrix(res2$r, res2$P)

res3|>
  mutate(test = paste0("r = ", cor, ", p = ", p))|>
  select(row, column, test)|>
  pivot_wider(names_from = column, values_from = test)
```
#### Individual

```{r message=FALSE, warning=FALSE}
correlation_df <- nirs_per_trial_plus_qst|>
  distinct(record_id, .keep_all = TRUE) |> 
  select(stai_state,stai_trait,anxiety_c,cbq_effortful_control,cbq_fear)


report(cor.test(correlation_df$stai_state, correlation_df$anxiety_c, method = "pearson", conf.level = 0.95))
report(cor.test(correlation_df$stai_state, correlation_df$cbq_effortful_control, method = "pearson", conf.level = 0.95))
report(cor.test(correlation_df$stai_state, correlation_df$cbq_fear, method = "pearson", conf.level = 0.95))
report(cor.test(correlation_df$stai_trait, correlation_df$anxiety_c, method = "pearson", conf.level = 0.95))
report(cor.test(correlation_df$stai_trait, correlation_df$cbq_effortful_control, method = "pearson", conf.level = 0.95))
report(cor.test(correlation_df$stai_trait, correlation_df$cbq_fear, method = "pearson", conf.level = 0.95))
report(cor.test(correlation_df$cbq_fear, correlation_df$cbq_effortful_control, method = "pearson", conf.level = 0.95))
report(cor.test(correlation_df$stai_state, correlation_df$stai_trait, method = "pearson", conf.level = 0.95))
report(cor.test(correlation_df$anxiety_c, correlation_df$cbq_fear, method = "pearson", conf.level = 0.95))
report(cor.test(correlation_df$anxiety_c, correlation_df$cbq_effortful_control, method = "pearson", conf.level = 0.95))
```

### PFC

#### State - Model -block 1 reference

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 3", "Block 1", "Block 2"))

model3_1 <- lmer(
  avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c * block + stai_state * block   +(1 + block + trial_num | record_id) ,
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model3_1)

# emm = emmeans(model3_1, ~ cbq_effortful_control*block )
# pairs(emm)
# # or for simple comparisons
# #pairs(emm, simple = "each")

anova(model3_1, type = "III")
report(model3_1)

model3_1_x <- lmer(
  avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state * block  +(1 + block +trial_num | record_id) ,
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)
model3_1_y <- lmer(
  avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state +(1 + block + trial_num| record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

performance::compare_performance(model3_1, model3_1_x, model3_1_y)
summary(model3_1_y)
report(model3_1_y)
### model_3_1 is the best model <- this was before correction
### model_3_1_y
```
##### Block 2 reference
```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 2", "Block 1", "Block 3"))

model3_1_y_b <- lmer(
  avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state +(1 + block + trial_num| record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

report(model3_1_y_b)
```

##### Report

```{r}
model3_1_y_for_report <- lmer(
  avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state + (1 + block + trial| record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model3_1_y_for_report)
tab_model(model3_1_y_for_report, p.val = "kr", show.df = TRUE)
```

##### Plot for interaction: EC

```{r}
interaction_eff3_1 <- ggpredict(model3_1_y, c("cbq_effortful_control", "block"))
interaction_eff3_1

interaction_eff3_1$group <- factor(interaction_eff3_1$group, levels = c("Block 2", "Block 1", "Block 3"))

ggplot(interaction_eff3_1, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Effortful Control (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "PFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_EC-Block_state_PFC.png", width = 10, height = 8)
```

##### Slope of EC for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_1_y, ~ block, var = "cbq_effortful_control")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
contrast(slopes, "poly") 
```

##### Slope of Fear for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_1_y, ~ block, var = "cbq_fear")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
contrast(slopes, "poly") 
```

#### Trait - Model -block 3 reference

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 3", "Block 1", "Block 2"))

model3_2 <- lmer(
  avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c * block + stai_trait * block +
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

summary(model3_2)

# emm = emmeans(model3_2, ~ cbq_effortful_control*block )
# pairs(emm)
# # or for simple comparisons
# #pairs(emm, simple = "each")

anova(model3_2, type = "III")
report(model3_2)

performance::check_outliers(model3_2)

model3_2_x <- lmer(
  avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_trait * block  +
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)
model3_2_y <- lmer(
  avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_trait  +
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

performance::compare_performance(model3_2, model3_2_x, model3_2_y)
summary(model3_2_y)
### model3_2 is the best model <- this was before correction
### model3_2_y
```

##### Slope of EC for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_2_y, ~ block, var = "cbq_effortful_control")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
contrast(slopes, "poly") 
```

##### Slope of Fear for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_2_y, ~ block, var = "cbq_fear")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
contrast(slopes, "poly") 
```

##### Plot for interaction: EC

```{r}
interaction_eff3_2 <- ggpredict(model3_2_y, c("cbq_effortful_control", "block"))
interaction_eff3_2

interaction_eff3_2$group <- factor(interaction_eff3_2$group, levels = c("Block 2", "Block 1", "Block 3"))

ggplot(interaction_eff3_2, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Effortful Control (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "PFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_EC-Block_trait_PFC.png", width = 10, height = 8)
```

##### Plot for interaction: F

```{r}
interaction_eff3_3 <- ggpredict(model3_2_y, c("cbq_fear", "block"))
interaction_eff3_3

interaction_eff3_3$group <- factor(interaction_eff3_3$group, levels = c("Block 2", "Block 1", "Block 3"))

ggplot(interaction_eff3_3, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Fear (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "PFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_F-Block_trait_PFC.png", width = 10, height = 8)
```

### DLPFC

#### State - Model -block 3 reference

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 3", "Block 1", "Block 2"))

model3_3 <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c * block + stai_state * block
    + (1 + block + trial_num | record_id) ,
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)



model3_3_x <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state * block  + 
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

model3_3_y <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state + 
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "nlminbwrap",
    optCtrl = list(maxfun = 5e5)
  )
)

performance::compare_performance(model3_3, model3_3_x, model3_3_y)

## best model is model3_3 <- this was before correction
## best model is model3_3_y
summary(model3_3_y)

# emm = emmeans(model3_3, ~ cbq_effortful_control*block )
# pairs(emm)
# # or for simple comparisons
# #pairs(emm, simple = "each")

anova(model3_3_y, type = "III")
report(model3_3_y)
```
###### Model Block 2 Reference
```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 2", "Block 1", "Block 3"))

model3_3_y_b <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state + 
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "nlminbwrap",
    optCtrl = list(maxfun = 5e5)
  )
)

report(model3_3_y_b)

```

###### Model Block 1 Reference
```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 1", "Block 2", "Block 3"))

model3_3_y_c <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state + 
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "nlminbwrap",
    optCtrl = list(maxfun = 5e5)
  )
)

report(model3_3_y_c)

```


##### Slope of EC for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_3_y, ~ block, var = "cbq_effortful_control")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
```

##### Slope of Fear for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_3_y, ~ block, var = "cbq_fear")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
contrast(slopes, "poly") 
```

##### Plot for interaction: EC

```{r}
interaction_eff3_3 <- ggpredict(model3_3_y, c("cbq_effortful_control", "block"))
interaction_eff3_3

interaction_eff3_3$group <- factor(interaction_eff3_3$group, levels = c("Block 2", "Block 1", "Block 3"))


ggplot(interaction_eff3_3, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Effortful Control (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "DLPFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_EC-Block_state_DLPFC.png", width = 10, height = 8)
```

##### Plot for interaction: Fear

```{r}
interaction_eff3_3 <- ggpredict(model3_3_y, c("cbq_fear", "block"))
interaction_eff3_3

interaction_eff3_3$group <- factor(interaction_eff3_3$group, levels = c("Block 2", "Block 1", "Block 3"))


ggplot(interaction_eff3_3, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Fear (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "DLPFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_F-Block_state_DLPFC.png", width = 10, height = 8)
```

#### Trait - Model -block 3 reference

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 3", "Block 1", "Block 2"))

model3_4 <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c * block + stai_trait * block +  (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)




model3_4_x <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_trait * block + 
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

model3_4_y <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_trait + 
    (1 + block + trial_num | record_id) ,
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

performance::compare_performance(model3_4, model3_4_x, model3_4_y)
performance::check_outliers(model3_4_y)
summary(model3_4_y)

# emm = emmeans(model3_4, ~ cbq_effortful_control*block )
# pairs(emm)
# # or for simple comparisons
# #pairs(emm, simple = "each")

anova(model3_4_y, type = "III")
report(model3_4_y)
```

##### model  Bock 2 reference
```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 2", "Block 1", "Block 3"))

model3_4_y_b <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_trait + 
    (1 + block + trial_num | record_id) ,
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

report(model3_4_y_b)
```


##### Slope of EC for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_4_y, ~ block, var = "cbq_effortful_control")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
```
##### model  Bock 1 reference
```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 1", "Block 2", "Block 3"))

model3_4_y_c <- lmer(
  DLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_trait + 
    (1 + block + trial_num | record_id) ,
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

report(model3_4_y_c)
```
##### Slope of Fear for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_4_y, ~ block, var = "cbq_fear")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
```

##### Plot for interaction: Fear

```{r}
interaction_eff3_4 <- ggpredict(model3_4_y, c("cbq_fear", "block"))
interaction_eff3_4

interaction_eff3_4$group <- factor(interaction_eff3_4$group, levels = c("Block 2", "Block 1", "Block 3"))

ggplot(interaction_eff3_4, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Fear (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "DLPFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_F-Block_trait_DLPFC.png", width = 10, height = 8)
```

##### Plot for interaction: EC

```{r}
interaction_eff3_4_b <- ggpredict(model3_4_y, c("cbq_effortful_control", "block"))
interaction_eff3_4_b

interaction_eff3_4_b$group <- factor(interaction_eff3_4_b$group, levels = c("Block 2", "Block 1", "Block 3"))

ggplot(interaction_eff3_4_b, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Effortful Control (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "DLPFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_EC-Block_trait_DLPFC.png", width = 10, height = 8)
```

### VLPFC

#### State - Model -block 3 reference

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 3", "Block 1", "Block 2"))


model3_5 <- lmer(
  VLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c * block + stai_state * block + 
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)



model3_5_x <- lmer(
  VLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state * block + 
    (1 + block + trial_num | record_id) ,
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

model3_5_y <- lmer(
  VLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state + 
    (1 + block + trial_num | record_id) ,
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

performance::compare_performance(model3_5, model3_5_x, model3_5_y)

## best model is model3_5 <-
## 
## best model is model3_5_y <-this is after correction


summary(model3_5_y)

# emm = emmeans(model3_5, ~ cbq_effortful_control*block )
# pairs(emm)
# # or for simple comparisons
# #pairs(emm, simple = "each")

anova(model3_5_y, type = "III")
report(model3_5_y)
```

#### Model 1 Block reference:
```{r}

nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 1", "Block 2", "Block 3"))

model3_5_y_b <- lmer(
  VLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_state + 
    (1 + block + trial_num | record_id) ,
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

report(model3_5_y_b)
```


##### Plot for Main Effect: STAI State

```{r}
main_eff3_5 <- ggpredict(model3_5_y, c("stai_state"))
main_eff3_5

ggplot(main_eff3_5, aes(x, predicted)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_x_continuous(
    name = "Parent State Anxiety"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "VLPFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_state_VLPFC.png", width = 10, height = 8)
```

##### Slope of EC for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_5_y, ~ block, var = "cbq_effortful_control")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
```

##### Slope of F for each block

```{r}
# Slope of F for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_5_y, ~ block, var = "cbq_fear")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
```

##### Plot for Interaction: Fear

```{r}
interaction_eff3_5 <- ggpredict(model3_5_y, c("cbq_fear", "block"))
interaction_eff3_5

interaction_eff3_5$group <- factor(interaction_eff3_5$group, levels = c("Block 2", "Block 1", "Block 3"))

ggplot(interaction_eff3_5, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Fear (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "VLPFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_F-Block_state_VLPFC.png", width = 10, height = 8)
```

##### Plot for Interaction: EC

```{r}
interaction_eff3_5_b <- ggpredict(model3_5_y, c("cbq_effortful_control", "block"))
interaction_eff3_5_b

interaction_eff3_5_b$group <- factor(interaction_eff3_5_b$group, levels = c("Block 2", "Block 1", "Block 3"))


ggplot(interaction_eff3_5_b, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Effortful Control (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "VLPFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02))  # Setting Y-axis limits and ticks


ggsave("figures/aim_3_EC-Block_state_VLPFC.png", width = 10, height = 8)
```

#### Trait - Model -block 3 reference

```{r}
nirs_per_trial_plus_qst$block <- factor(nirs_per_trial_plus_qst$block, levels = c("Block 3", "Block 1", "Block 2"))

model3_6 <- lmer(
  VLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c * block + stai_trait * block +
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)


performance::check_outliers(model3_6)


model3_6_x <- lmer(
  VLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_trait * block + 
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

model3_6_y <- lmer(
  VLPFC_avg_score ~ c_age_y_centered +
    c_sex + cbq_fear * block + cbq_effortful_control * block + anxiety_c + stai_trait  +
    (1 + block + trial_num | record_id),
  data = nirs_per_trial_plus_qst,
  control = lmerControl(
    optimizer = "bobyqa",
    optCtrl = list(maxfun = 5e5)
  )
)

performance::compare_performance(model3_6, model3_6_x, model3_6_y)

## the best model is model3_6 <-
## the best model is model3_6_y <- after correction
## 
summary(model3_6_y)

# emm = emmeans(model3_6, ~ cbq_effortful_control*block )
# pairs(emm)
# # or for simple comparisons
# #pairs(emm, simple = "each")

report(model3_6_y)

```

##### Slope of EC for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_6_y, ~ block, var = "cbq_effortful_control")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
```

##### Slope of F for each block

```{r}
# Slope of EC for each block
emm_options(lmer.df = "satterthwaite")
slopes <- emtrends(model3_6_y, ~ block, var = "cbq_fear")
summary(slopes, infer = c(TRUE, TRUE))

# Pairwise tests: are the EC slopes different between blocks?
pairs(slopes, adjust = "none")  # or "fdr"
```

##### Plot for Interaction: Fear

```{r}
interaction_eff3_6 <- ggpredict(model3_6_y, c("cbq_fear", "block"))
interaction_eff3_6

interaction_eff3_6$group <- factor(interaction_eff3_6$group, levels = c("Block 2", "Block 1", "Block 3"))

ggplot(interaction_eff3_6, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Fear (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "VLPFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_F-Block_trait_VLPFC.png", width = 10, height = 8)
```

##### Plot for Interaction: EC

```{r}
interaction_eff3_6_b <- ggpredict(model3_6_y, c("cbq_effortful_control", "block"))
interaction_eff3_6_b

interaction_eff3_6_b$group <- factor(interaction_eff3_6_b$group, levels = c("Block 2", "Block 1", "Block 3"))

ggplot(interaction_eff3_6_b, aes(x, predicted, color = group)) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, color = NA) +
  theme_classic() +
  scale_color_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_fill_manual(
    name = "Block Type",
    values = natparks.pals("Saguaro", 3),
    labels = c(
      "Block 1" = "Baseline",
      "Block 2" = "Stressful",
      "Block 3" = "Recovery"
    )
  ) +
  scale_x_continuous(
    name = "Effortful Control (CBQ scores)"
  ) +
  theme(
    axis.text = element_text(size = 19),
    axis.title = element_text(size = 20),
    legend.text = element_text(size = 19),
    legend.title = element_text(face = "bold", size = 19),
    legend.title.align = 0.5
  ) +
  scale_y_continuous(name = "VLPFC Neural Synchrony (predicted)", limits = c(0.2, .32), breaks = seq(0.2, .32, by = 0.02)) # Setting Y-axis limits and ticks


ggsave("figures/aim_3_EC-Block_trait_VLPFC.png", width = 10, height = 8)
```

### Graph: Anxiety Children vs. State Anxiety Parent

```{r}
nirs_per_trial_plus_qst %>%
  distinct(record_id, .keep_all = TRUE) %>%
  select(record_id, DLPFC_avg_score, anxiety_c, stai_state, stai_trait) %>%
  ggplot(., aes(anxiety_c, DLPFC_avg_score)) +
  geom_point(size = 3) +
  labs(x = "Caregiver State Anxiety", y = "Dyadic Neural Synchrony Values") +
  geom_smooth(method = "lm") +
  scale_fill_brewer(palette = "Dark2") +
  scale_colour_brewer(palette = "Dark2") +
  guides(fill = FALSE, col = FALSE) +
  theme_apa() +
  theme(text = element_text(size = 25))

ggsave("figures/aim_3_corr_child_parent_state_anx.png", width = 12, height = 8)
```

### Graph: Anxiety Children vs. State Anxiety Parent

```{r}
nirs_per_trial_plus_qst %>%
  distinct(record_id, .keep_all = TRUE) %>%
  select(record_id, DLPFC_avg_score, anxiety_c, stai_state, stai_trait) %>%
  ggplot(., aes(stai_state, anxiety_c)) +
  geom_point(size = 3) +
  labs(x = "Caregiver State Anxiety", y = "Child Anxiety") +
  geom_smooth(method = "lm") +
  scale_fill_brewer(palette = "Dark2") +
  scale_colour_brewer(palette = "Dark2") +
  guides(fill = FALSE, col = FALSE) +
  theme_apa() +
  theme(text = element_text(size = 25))

ggsave("figures/aim_3_corr_child_parent_state_anx.png", width = 12, height = 8)
```

### Graph: Anxiety Children vs. Trait Anxiety Parent

```{r}
nirs_per_trial_plus_qst %>%
  distinct(record_id, .keep_all = TRUE) %>%
  select(record_id, DLPFC_avg_score, anxiety_c, stai_state, stai_trait) %>%
  ggplot(., aes(stai_trait, anxiety_c)) +
  geom_point(size = 3) +
  labs(x = "Caregiver Trait Anxiety", y = "Child Anxiety Score") +
  geom_smooth(method = "lm") +
  scale_fill_brewer(palette = "Dark2") +
  scale_colour_brewer(palette = "Dark2") +
  guides(fill = FALSE, col = FALSE) +
  theme_apa() +
  theme(text = element_text(size = 25))

ggsave("figures/aim_3_corr_child_parent_trait_anx.png", width = 12, height = 8)
```

### Report-State

```{r}
tab_model(model3_1_y, model3_3_y, model3_5_y, p.val = "kr", show.df = TRUE, collapse.ci = TRUE, p.style = "stars", digits = 4)
```

### Report-Trait

```{r}
tab_model(model3_2_y, model3_4_y, model3_6_y, p.val = "kr", show.df = TRUE, collapse.ci = TRUE, p.style = "stars", digits = 4)
```

## 4. Correlations with all

```{r}
report(cor.test(block1$stai_state, block1$cbq_fear, method = "pearson", conf.level = 0.95))
report(cor.test(block1$stai_trait, block1$cbq_fear, method = "pearson", conf.level = 0.95))
report(cor.test(block1$anxiety_c, block1$cbq_fear, method = "pearson", conf.level = 0.95))
report(cor.test(block1$stai_state, block1$cbq_effortful_control, method = "pearson", conf.level = 0.95))
report(cor.test(block1$stai_trait, block1$cbq_effortful_control, method = "pearson", conf.level = 0.95))
report(cor.test(block1$anxiety_c, block1$cbq_effortful_control, method = "pearson", conf.level = 0.95))
report(cor.test(block1$stai_state, block1$stai_trait, method = "pearson", conf.level = 0.95))
report(cor.test(block1$stai_trait, block1$anxiety_c, method = "pearson", conf.level = 0.95))
report(cor.test(block1$stai_state, block1$anxiety_c, method = "pearson", conf.level = 0.95))
report(cor.test(block1$cbq_effortful_control, block1$anxiety_c, method = "pearson", conf.level = 0.95))
```
